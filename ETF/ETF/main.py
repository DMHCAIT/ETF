...
class ETFTradingSystem:
    ...
    def _handle_trading_signal(self, signal: Dict[str, Any]) -> None:
        """
        Handle trading signals from strategy (ML-enhanced or traditional).
    ...
    async def _run_periodic_tasks(self) -> None:
        ...
        try:
            ...
            # Check for position management (stop-loss, profit targets)
    ...
    def _check_position_management(self) -> None:
        """Check existing positions for stop-loss and profit targets."""
        try:
            ...
            for position in positions:
                ...
                # Calculate return
                price_return = (current_price - avg_price) / avg_price
                ...
                if price_return >= self.trading_strategy.sell_target:
                    should_sell = True
                    sell_reason = f"Profit target reached: {price_return*100:.2f}%"
                
                # Stop-loss triggered
                elif price_return <= self.trading_strategy.stop_loss:
                    should_sell = True
                    sell_reason = f"Stop-loss triggered: {price_return*100:.2f}%"
                ...
                if should_sell:
                    ...
                    self._handle_trading_signal(sell_signal)
