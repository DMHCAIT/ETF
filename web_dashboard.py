...
trading_parameters = {
    'buy_threshold': -0.01,      # -1% (Trigger buy when price drops 1% from previous close)
    'sell_target': 0.02,         # +2% (Auto-sell at 2% profit)
    'loss_alert': -0.05,         # -5% (Alert only, no auto-sell at 5% loss)
    ...
    'one_buy_per_day': True,     # Only one buy order per ETF per day
    'auto_sell_on_loss': False   # Don't auto-sell on loss, only alert
}
# Global variables
config = None
...
# =================== DAILY AUTHENTICATION ROUTES ===================

@app.route('/api/auth/generate-login', methods=['POST'])
def generate_zerodha_login():
    """Generate Zerodha login URL for daily authentication"""
    try:
        data = request.get_json()
        ...
        # Store API key temporarily for token generation
        app.config['TEMP_API_KEY'] = api_key
        
        return jsonify({
            'status': 'success',
            'login_url': login_url,
            'message': 'Click the login URL and copy the request_token'
        })
        
    except Exception as e:
        logging.error(f"Login URL generation error: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/auth/activate-live', methods=['POST'])
def activate_live_trading():
    """Activate live trading with request token"""
    try:
        data = request.get_json()
        api_secret = data.get('api_secret')
        request_token = data.get('request_token')
        
        if not all([api_secret, request_token]):
            return jsonify({'status': 'error', 'message': 'API secret and request token required'}), 400
        
        api_key = app.config.get('TEMP_API_KEY')
        if not api_key:
            return jsonify({'status': 'error', 'message': 'API key not found. Generate login URL first'}), 400
        ...
        # Update environment variables for live trading
        live_credentials = {
            'DEVELOPMENT_MODE': 'false',
            'USE_MOCK_DATA': 'false',
            'BROKER_API_KEY': api_key,
            'BROKER_SECRET_KEY': api_secret,
        ...
        # Update or add live trading credentials
        updated_keys = set()
        for i, line in enumerate(env_lines):
            for key, value in live_credentials.items():
        ...
        try:
            # This would update Vercel env vars - simplified for demo
            pass
        except Exception as e:
            logging.warning(f"Could not update Vercel env vars: {e}")
        
        # Clear temporary API key
        app.config.pop('TEMP_API_KEY', None)
        
        # Update trading data with live account info
        trading_data['account_info'].update({
            'user_name': profile.get('user_name', 'N/A'),
            'user_id': profile.get('user_id', 'N/A'),
            'email': profile.get('email', 'N/A'),
            'total_balance': margins.get('equity', {}).get('net', 0),
        ...
        # Notify WebSocket clients about live mode activation
        socketio.emit('live_mode_activated', {
            'status': 'Live trading activated',
            'account': profile.get('user_name'),
            'balance': margins.get('equity', {}).get('net', 0),
            'timestamp': datetime.now().isoformat()
        })
        
        # Log successful activation
        logging.info(f"Live trading activated for user: {profile.get('user_name')}")
        ...
        logging.error(f"Live trading activation error: {e}")
        return jsonify({'status': 'error', 'message': f'Activation failed: {str(e)}'}), 500
...
def get_auth_status():
    ...
    try:
        ...
        has_access_token = bool(os.getenv('BROKER_ACCESS_TOKEN')) and os.getenv('BROKER_ACCESS_TOKEN') != 'placeholder_token'
        
        return jsonify({
            'status': 'success',
            'development_mode': development_mode,
            'live_mode': not development_mode and has_access_token,
...
@app.route('/api/auth/switch-demo', methods=['POST'])
def switch_to_demo():
    """Switch back to demo mode"""
    try:
        # Update environment to demo mode
        demo_credentials = {
            'DEVELOPMENT_MODE': 'true',
            'USE_MOCK_DATA': 'true',
            'BROKER_ACCESS_TOKEN': 'placeholder_token'
        }
        ...
        # Reset account info to demo
        trading_data['account_info'].update({
            'user_name': 'Demo User',
            'user_id': 'DEMO123',
            'email': 'demo@example.com',
            'total_balance': 326637.87,
            'available_cash': 300000.00,
            'used_margin': 26637.87,
            'live_mode': False,
            'session_start': datetime.now().isoformat()
        })
        
        # Notify clients
        socketio.emit('demo_mode_activated', {
        ...
        logging.error(f"Demo mode switch error: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
